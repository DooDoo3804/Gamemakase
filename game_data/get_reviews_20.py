...
def get_times():
    ...
    # 하나로 통합한 파일 생성
    with open("final20\\total_reviews.json", 'w', encoding="UTF-8") as make_file:
        json.dump(total_reviews_list, make_file,
                  ensure_ascii=False, indent="\t")

    # 예외 파일 생성
    with open("final20\\exceptions.json", 'w', encoding="UTF-8") as make_file:
